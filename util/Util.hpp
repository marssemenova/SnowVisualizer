 ...
 */
inline int getRandInt(float min, float max) {
    if (min == max) {
        return min;
    }
 ...
 */
inline void calcNormal(float* trig, float* norm) {
    for (int x = 0; x < 9; x+=3) {
 ...
 */
inline void findPointInTriangle(float* trig, float pt[3]) {
    float x1 = trig[0], y1 = trig[1], z1 = trig[2], x2=trig[3], y2=trig[4], z2=trig[5], x3=trig[6], y3=trig[7], z3=trig[8];
    float v1x=x1-x2, v1y=y1-y2, v1z=z1-z2;
    float v2x=x3-x2, v2y=y3-y2, v2z=z3-z2;
    float s = getRandFloat(0, 1), t = getRandFloat(0, 1);
    if (s + t <= 1) {
        pt[0] = s*v1x + t*v2x;
        pt[1] = s*v1y + t*v2y;
        pt[2] = s*v1z + t*v2z;
